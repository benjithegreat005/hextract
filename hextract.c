...
void print_line(uint8_t* buffer, int bytes, int offset, int line_len) {
    //Uncomment bellow line to print line numbers alongside extracted hex
    //printf("%d |", offset);
...
void dump_file(FILE* file, int offset, int seg_len, int line_len) {
    long fsize;
    ...
    while (true) {
        ...
        if (bytes > 0) {
            print_line(buffer, bytes, offset, line_len);
            offset += bytes;
            seg_len -= bytes;
        } else {
            break;
...
int main(int argc, char *argv[]) {
    ...
    printf("\n");
    int line_len = 16;
    dump_file(file, offset, seg_len, line_len);
